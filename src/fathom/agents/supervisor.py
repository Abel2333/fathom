...
async def supervisor(state: SupervisorState):
    ...
    logger.debug(f"Available tools: {[t.__name__ if hasattr(t, '__name__') else str(t) for t in lead_researcher_tools]}")
    ...
    if hasattr(response, "tool_calls") and response.tool_calls:
        ...
        logger.info(f"✓ Supervisor decided to call {len(response.tool_calls)} tool(s): {', '.join(tool_names)}")
        for i, tc in enumerate(response.tool_calls, 1):
            logger.debug(f"  Tool #{i}: {tc.get('name')} with args: {str(tc.get('args', {}))[:100]}")
    else:
        logger.warning("⚠ No tool calls generated by supervisor")

    # Updatte state and proceed to tool execution
    logger.info(f"→ Routing to: supervisor_tools (iteration will be {current_iteration + 1})")
